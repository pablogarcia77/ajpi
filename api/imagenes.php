...
header("Access-Control-Max-Age: 60");
 ...
 switch ($_SERVER['REQUEST_METHOD']) {
  ...
  case 'POST':
    ...
    if($validar==1) {
      $imagen = save_base64_image($array['encodedImage'], "../ajpiimagenes/" . strtoupper(md5($array['encodedImage'])));

      //extension
      $splited = explode(',', substr( $array['encodedImage'] , 5 ) , 2);
      $mime=$splited[0];
      ...
      if($imagen!=null) {
        ...
        print_json(200, "Existe", 'http://localhost/ajpi/ajpiimagenes/'.strtoupper(md5($array['encodedImage'])).'.'.$exten);
      }
    }else{
 ...
 function save_base64_image($base64_image_string, $output_file_without_extentnion, $path_with_end_slash="" ) {
     ...
     if(file_exists($output_file_with_extentnion)) {
      ...
      $salida = 'http://localhost/ajpi/ajpiimagenes/'.$corto[2];
      return $salida;
     }
